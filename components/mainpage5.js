...
export default function Main5({ topOrdersData }) {
  ...
  const fetchData = async () => {
    try {
      ...
      //   `https://api.mulltiply.com/offers/active-offers-stats-new/${uri}?type=topOrders`
      // );
      if (topOrdersData?.length > 0) {
        setTitle(topOrdersData[0]?.category);
        setResponseData(topOrdersData[0]?.items || []);
        console.log("fetching responseDatafor MainPage5", topOrdersData[0]);
      }
    } catch (error) {
  ...
  return (
    <div className="py-1 sm:w-[639px] md:w-[767px] lg:w-full mb-20">
      <div className="flex flex-row justify-between items-center px-2">
        ...
        <Link href={`/items/${"topOrders"}`}>
          <p className="text-green-600 text-sm">see all</p>
        </Link>
      </div>
      <div className="px-1 py-1 h-[326px] md:h-80 flex flex-row gap-1 overflow-x-scroll overflow-y-hidden w-full">
        <div className="px-1 py-1 bg-white h-[250px] flex flex-row gap-1">
          {responseData &&
            responseData.map((item, index) => {
              ...
              return (
                ...
                >
                    ...
                    <div className="flex justify-end pr-2 pt-1">
                      {
                        ...
                        >
                          <img
                            src={
                              isClicked ? "./filledheart.png" : "./heart.png"
                            }
                            className={`w-4 h-4 object-contain ${isClicked ? "animate-grow" : ""
                              }`}
                          />
                        </div>
                      }
                  ...
                  <div className="flex mt-1 flex-col h-[30%] py-1 px-1">
